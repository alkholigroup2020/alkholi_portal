...
router.post('/add-dtr-user', auth, async (req, res) => {
  ...
  try {
    ...
    const checkUser = await portalDBConnection
      .request()
      .query(`exec dtr_users_checkIfExist '${req.body.code}'`)
    ...
    if (checkUser.recordset[0].exist === 1) {
      ...
      await portalDBConnection.request().query(`
        exec dtr_users_addData '${memberInfo.recordset[0].employee_code}',
...
router.post('/delete-dtr-user', auth, async (req, res) => {
  const portalDBConnection = await portalDB()
  try {
    const del = await portalDBConnection
      .request()
      .query(`exec dtr_users_deleteMember '${req.body.code}'`)
