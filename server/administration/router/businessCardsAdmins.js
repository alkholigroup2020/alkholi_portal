...
router.post('/add-business-card-admin', auth, async (req, res) => {
  try {
    // check if the member already exist
    await sql.connect(sqlConfigs)
    const checkUser =
      await sql.query`exec business_card_admins_checkIfExist ${req.body.code}`
    ...
    if (checkUser.recordset[0].exist === 1) {
      ...
      await sql.connect(sqlConfigs)
      await sql.query`
        exec business_card_admins_addData ${memberInfo.recordset[0].employee_code},
...
router.post('/delete-business-card-admin', auth, async (req, res) => {
  try {
    await sql.connect(sqlConfigs)
    const del =
      await sql.query`exec business_card_admins_deleteMember ${req.body.code}`
