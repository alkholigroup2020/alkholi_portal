  ...
  <v-container fluid class="pt-0">
    ...
    <v-row class="my-0">
      <v-col cols="12" sm="8" lg="6" xl="4" class="py-0">
        <v-form @submit.prevent="addDTRUser">
          <div class="d-flex">
            <v-text-field
              v-model="dtrUser"
      ...
      <v-col cols="12" class="py-0">
        ...
        >
          <template #default>
            ...
            <tbody>
              <tr v-for="member in dtrUsers" :key="member._id">
                ...
                <td>
                  <dtrUserDelete :employee="member.employeeID" />
                </td>
              </tr>
            </tbody>
          </template>
...
export default {
  ...
  computed: {
    ...mapState({
      dtrUsers: (state) => state.administration.dtrUsers.dtrUsers,
    }),
  },
  ...
  methods: {
    async getDTRUsers() {
      try {
        await this.$store.dispatch('administration/dtrUsers/getDTRUsers')
      } catch (e) {
      ...
      try {
        ...
        if (A !== '') {
          this.$nextTick(async () => {
            ...
            await this.$store.dispatch(
              'administration/dtrUsers/addDTRUser',
              employeeCode
            )
            this.dtrUser = ''
            await this.getDTRUsers()
